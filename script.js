/* =====================================================================
   BOLEST.AI — ROBUST BOOTSTRAP
   ===================================================================== */
(() => {
  function initBolestApp() {
    if (!window.supabase || typeof window.supabase.createClient !== "function") {
      console.error("BOLEST.AI: Supabase SDK is not loaded.");
      const note = document.getElementById("auth-note");
      if (note) {
        note.textContent = "Не вдалося завантажити Supabase. Оновіть сторінку.";
      }
      return;
    }
    ...
    function renderActiveQuestion() {
      ...
      document.getElementById("session-question-text").textContent = q.question;
  
      const optionsDiv = document.getElementById("session-options");
      optionsDiv.innerHTML = "";
  
      ['a', 'b', 'c', 'd'].forEach(letter => {
        if (!q[`option_${letter}`]) return;
        const opt = document.createElement("div");
        opt.className = "session-option";
        opt.innerHTML = `<div class="session-option-letter">${letter.toUpperCase()}</div><span>${q[`option_${letter}`]}</span>`;
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initBolestApp, { once: true });
  } else {
    initBolestApp();
  }
})();
